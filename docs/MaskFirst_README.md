...
**Still deliberately left untranslated**: the `REGEX_PRESETS_DEFAULT` rule `name`/`example` fields (technical/reference labels, not detection input). `localStorage` persistence here (and for Region/Persona, §2.1) is a deliberate, scoped exception to MaskFirst's data-lifecycle promise (§1.2) — these are UI preferences, not document content or scan state, which are still destroyed on every reload.
